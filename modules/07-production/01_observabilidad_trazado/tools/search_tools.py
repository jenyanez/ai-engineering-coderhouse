"""Herramientas de búsqueda y consulta semántica sobre ChromaDB con trazabilidad para el Agente de Investigación."""
...
load_dotenv(Path(__file__).resolve().parents[3] / "pre_entrega_06" / ".env")
...
def query_chroma_vector_db(query: str, k: int = 3) -> str:
    """Realiza una búsqueda semántica por similitud de cosenos en la base vectorial ChromaDB.
...
def search_market_trends(query: str) -> str:
    ...
    """
    return query_chroma_vector_db.invoke({"query": query, "k": 2})
...
def query_tech_knowledge_base(topic: str) -> str:
    ...
    """
    return query_chroma_vector_db.invoke({"query": f"factores impulsores riesgos {topic}", "k": 2})
